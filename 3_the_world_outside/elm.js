...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var jsLikes = Elm.Native.Port.make(_elm).inboundSignal("jsLikes",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
      v);
   });
   var outbox = $Signal.mailbox("");
   var logger = Elm.Native.Port.make(_elm).outboundSignal("logger",
   function (v) {
      return v;
   },
   outbox.signal);
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([$Html.text("Likes: ")
              ,$Html.text($Basics.toString(model))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,Like)]),
              _U.list([$Html.text("Like")]))
              ,A2($Html.button,
   ...
   var initModel = {ctor: "_Tuple2"
                   ,_0: initialLikes
                   ,_1: $Effects.none};
   var update = F3(function (log,action,model) {
      var reset = initialLikes;
      var liked = model + 1;
      var _p0 = action;
      switch (_p0.ctor)
      {case "Like": return {ctor: "_Tuple2",_0: liked,_1: log(liked)};
         case "Reset": return {ctor: "_Tuple2",_0: reset,_1: log(reset)};
         default: return {ctor: "_Tuple2",_0: model,_1: $Effects.none};}
   });
   var updateWithLogging = update(log(outbox));
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,log: log
                             ,view: view
                             ,updateWithLogging: updateWithLogging
                             ,app: app
                             ,main: main
                             ,outbox: outbox
